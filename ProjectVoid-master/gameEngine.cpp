...
#include "bulletBase.h"  //子弹父类

//敌人类
#include "enemyRangedDrone.h"  //无人机怪
#include "enemyBot.h"  //机器怪
#include "enemyExplosiveBall.h"  //爆炸怪
#include "enemyBotElite.h"  //精英机器怪
#include "enemyRangedDroneElite.h"  //精英无人机怪

#include "weaponSmg.h"  //冲锋枪
#include "weaponSmart.h"  //智能武器
#include "weaponShotgun.h"  //猎枪
#include "weaponDmr.h"

#include "upgradeMod.h"  //升级
...
GameEngine::GameEngine(const QString &weapon, const QString &load_archive): weapon_type(weapon), enemy_gen_count(0), bullet_gen_count(0), phase(0), phase_count(0), countdown_count(0), level(0), data_count(0)
{
    ...
    m_background->setZValue(-0x3ffffff);  //设置图形堆叠顺序，Z值越大顺序越前，将背景设置为最低层
    m_scene->addItem(m_background);

    // 角色生成,设置m_background为m_play的父类，。这意味着当父项移动或变形时，其子项也会相应地进行相对的移动或变形。
    // 此外，父项也负责管理子项的生命周期，当父项被销毁时，其子项也会被自动删除。
    ...
    // 移动player至地图中央，地图中心点移动到原点
    m_player->moveBy(2400, 1350);
    m_background->moveBy(-2400, -1350);

    // 游戏阶段初始化
    // 数据初始化
    enemy_generate_rate = 7.5;  // 怪物生成速率
    melee_enemy_rate = 100;  // 混战生成速率
    ranged_enemy_rate = 0;  // 远程怪生成速率
    charge_enemy_rate = 0;  // 冲锋怪生成速率
    explosive_enemy_rate = 0;  // 爆炸怪生成速率
    elite_enemy_rate = 0;  // 精英怪生成速率
    // 计时器初始化，每10ms调用一次phaseUp()
...
void GameEngine::enemyGenerator()
{
    ...
    qreal dist;
    do {
        // 生成0~4800之间的随机数，且global()返回一个全局随机数生成器，并且是简化了随机数生成和线程安全的
    ...
    {
        if (ran_elite >= elite_enemy_rate)
            enemy = new EnemyBot(m_player, m_scene, this); // 生成近战敌人
        else
            enemy = new EnemyBotElite(m_player, m_scene, this);
    } else if (random > melee_enemy_rate && random <= melee_enemy_rate + ranged_enemy_rate)
    {
        if (ran_elite >= elite_enemy_rate)
            enemy = new EnemyRangedDrone(m_player, m_scene, this); // 生成远程敌人
        else
...
{
    phase_count++;
    if (phase_count < 6000) return; // 60s，使用变量判断来实现定时器的效果，这样可以将定时作用绑定到一个定时器对象上
    phase_count = 0;
    phase++;
    switch (phase) {
    case 1:
...
// 升级选择技能，每升级出现三个选项，将三个技能编号存入list，在其他方法中调用选择和初始化
...
// 存档，其中beginGroup()~endGroup()之间的Setting都被分为一组在ini文件中呈现
void GameEngine::saveArchive(int sign)
{
    ...
    // 敌人存储，QVvariantList存储不同类型的值
    QVariantList enemyList;
    // items()方法已经过时，相当于调用项items(QRectF(x,y,w,h),mode)
    // items()函数返回一个QList<QGraphicsItem*>对象，其中包含了场景中的所有图形项。
    // 每个图形项都是QGraphicsItem*类型的指针，可以通过该指针获取和操作对应的图形项。
...
void GameEngine::showArchive()
{
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        archive_mod[i]->moveToThread(m_archiveThread);
    }
}
