...
StartMenu::StartMenu()
{
    ...
    // 将样式表读取到一个字符串中，并使用QLatin1String类来处理Latin1编码
    ...
    // 先进行连接后再初始化，这种写法是存在一些问题的，但该操作所花费时间忽略不计，故没有明显bug
    connect(prp_shop_btn, &QPushButton::clicked, this, [=](){
        setScene(s_shop);
    });
    // 设置武器选择，分别创建了4个WeaponMod对象
...
//    int hp_increment_lvl=1,muzzle_ckoke_lvl=1,data_collector_lvl=1,respawn_udisk_amount=1;
